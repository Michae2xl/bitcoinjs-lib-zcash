...
describe('zcashjs-lib (transactions with psbt)', () => {
    ...
    psbt.addInput({
      ...
      nonWitnessUtxo: Buffer.from(
        '0200000001f9f34e95b9d5c8abcd20fc5bd4a825d1517be62f0f775e5f36da944d9' +
          ...
          // value in zatoshis (Int64LE) = 0x015f90 = 90000
          '905f010000000000' +
          // scriptPubkey length
          '19' +
          // scriptPubkey
  ...
  it('can create (and broadcast via 3PBP) a typical Transaction', async () => {
    ...
    // to build and broadcast to the actual Zcash network, see https://github.com/bitcoinjs/zcashjs-lib/issues/839
  });
  ...
  it('can create (and broadcast via 3PBP) a Transaction with an OP_RETURN output', async () => {
    ...
    const data = Buffer.from('zcashjs-lib', 'utf8');
  ...
  it('can create (and broadcast via 3PBP) a Transaction, w/ a P2SH(P2WPKH) input', async () => {
    ...
    const inputData2 = await getInputData(5e4, p2sh.payment, true, 'p2sh');
    {
      const {
        hash,
        index,
        witnessUtxo, // NEW: this is an object of the output being spent { script: Buffer; value: zatoshis; }
        redeemScript,
      } = inputData;
      assert.deepStrictEqual(

...
import * as bitcoin from 'zcashjs-lib';
import { Taptree } from 'zcashjs-lib/src/types';
import {
  LEAF_VERSION_TAPSCRIPT,
  tapleafHash,
} from 'zcashjs-lib/src/payments/bip341';
import {
  toXOnly,
  tapTreeToList,
  tapTreeFromList,
} from 'zcashjs-lib/src/psbt/bip371';
import { witnessStackToScriptWitness } from 'zcashjs-lib/src/psbt/psbtutils';
...
describe('zcashjs-lib (transaction with taproot)', () => {
  it('can verify the BIP86 HD wallet vectors for taproot single sig (& sending example)', async () => {
    ...
    // Send some zats to the address via faucet. Get the hash and index. (txid/vout)
    const { txId: hash, vout: index } = await regtestUtils.faucetComplex(
      Buffer.from(output),
      amount,
    );
    // Sent 420000 zats to taproot address
...
class TaprootMultisigWallet {
  ...
  constructor(
    ...
    this.network = zcash.networks.bitcoin;

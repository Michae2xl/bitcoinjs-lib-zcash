...
const DEFAULT_OPTS: PsbtOpts = {
   ...
   * It is only here as a last ditch effort to prevent sending a 500 ZEC fee etc.
   */
  maximumFeeRate: 5000, // zatoshi per byte
};
...
export class Psbt {
  ...
  setMaximumFeeRate(zatoshiPerByte: number): void {
    check32Bit(zatoshiPerByte); // 42.9 ZEC per byte IS excessive... so throw
    this.opts.maximumFeeRate = zatoshiPerByte;
  }
 ...
 * It contains a zcashjs-lib Transaction object.
 */
...
function checkFees(psbt: Psbt, cache: PsbtCache, opts: PsbtOpts): void {
  ...
  const zatoshis = feeRate * vsize;
  if (feeRate >= opts.maximumFeeRate) {
    throw new Error(
      `Warning: You are paying around ${(zatoshis / 1e8).toFixed(8)} in ` +
        `fees, which is ${feeRate} zatoshi per byte for a transaction ` +

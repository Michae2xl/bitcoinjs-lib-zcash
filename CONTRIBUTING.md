...
To contribute a patch, the workflow is as follows:
  ...
  1. Submit a pull request to https://github.com/bitcoinjs/zcashjs-lib
...
## We adhere to zcash policy
Bitcoin script payment/script templates are based on community consensus,  but typically adhere to zcash node policy by default.

- `bitcoinjs.script.decompile` is consensus bound only,  it does not reject based on policy.
- `bitcoinjs.script.compile` will try to adhere to zcash `IsStandard` policies rules. (eg. minimalpush in https://github.com/bitcoinjs/zcashjs-lib/pull/638)
...
#### TLDR
Where "standards compliant" refers to the default policies of zcash,  we adhere to the following:
